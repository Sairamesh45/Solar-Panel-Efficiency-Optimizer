...
const MyMaintenanceRequests = ({ userId }) => {
  ...
  if (loading) return <div style={{ padding: '40px', textAlign: 'center', color: '#7f8c8d', fontSize: '1.1rem' }}>Loading maintenance requests...</div>;
  if (!requests.length) return (
    <div style={{ 
      ...
      <p style={{ margin: 0, opacity: 0.9 }}>No maintenance requests yet</p>
    </div>
  );
            ...
            <div style={{ position: 'relative' }}>
              ...
              {req.statusTimeline && req.statusTimeline.length > 0 && (
                <div style={{ 
                  ...
                  <h4 style={{ margin: '0 0 12px 0', fontSize: '1rem', color: '#2b7bbf', fontWeight: '800' }}>
                    Timeline History
                  </h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: 6 }}>
                    {req.statusTimeline.map((event, idx) => (
                      <div key={idx} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                        <div style={{ width: 12, display: 'flex', justifyContent: 'center' }}>
                          <div style={{ width: 8, height: 8, borderRadius: 8, background: '#f39c12', marginTop: 6 }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                            <strong style={{ color: '#122436' }}>{event.status.replace('_', ' ').toUpperCase()}</strong>
                          ...
                          {event.notes && <div style={{ marginTop: '2px', color: '#495057', fontSize: '0.9rem' }}>{event.notes}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
